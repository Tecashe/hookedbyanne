...
//     console.log("[v0] Has variants:", hasVariants)
//     console.log("[v0] Images:", images)
//     console.log("[v0] Variants:", variants)

//     // Add images to formData
//     images.forEach((img) => {
//       console.log("[v0] Adding image:", img)
...
//       const lowestPrice = variants.length > 0 ? Math.min(...variants.map((v) => v.price || 0)) : 0
//       formData.set("price", lowestPrice.toString())
//       console.log("[v0] Setting base price:", lowestPrice)

//       // Set stock to total of all variant stocks
//       const totalStock = variants.reduce((sum, v) => sum + (v.stock || 0), 0)
//       formData.set("stock", totalStock.toString())
//       console.log("[v0] Setting total stock:", totalStock)
//     }

//     try {
//       console.log("[v0] Calling createProduct/updateProduct")
//       let result
//       if (product) {
...
//       description:
//         "Without Vercel Blob integration, please use image URLs instead of uploading files. Uploaded files will not persist after deployment.",
//       variant: "destructive",
//     })
...
type ProductVariant = {
  id?: string
  color: string | null // Allow null to match database schema
  colorHex: string | null // Allow null to match database schema
  size: string | null // Allow null to match database schema
  sku: string | null // Allow null to match database schema
  price: number | null // Allow null to match database schema
  stock: number
  images: string[]
}
...
export function ProductForm({ product }: { product?: Product }) {
  ...
  const [colors, setColors] = useState<Array<{ name: string; hex: string }>>(() => {
    if (product?.variants && product.variants.length > 0) {
      const uniqueColors = Array.from(
        new Map(
          product.variants
            .filter((v) => v.color && v.colorHex) // Filter out null values
            .map((v) => [v.color, { name: v.color!, hex: v.colorHex! }]),
        ).values(),
      )
      return uniqueColors.length > 0 ? uniqueColors : [{ name: "", hex: "#FF6B6B" }]
    }
    return [{ name: "", hex: "#FF6B6B" }]
  })
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, variantIndex?: number) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to upload images. Please check your Blob integration and try again.",
        variant: "destructive",
      })
    } finally {
      setUploadingImages(false)
