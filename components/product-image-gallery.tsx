...
export function ProductImageGallery({ product, selectedVariantId }: ProductImageGalleryProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Main Image - Professional styling with shadow */}
      <div className="relative aspect-square overflow-hidden rounded-2xl bg-muted group shadow-xl">
        <Image
          src={validImages[selectedImage] || "/placeholder.svg"}
          alt={`${product.name} - Image ${selectedImage + 1}`}
          fill
        ...
        {validImages.length > 1 && (
          <>
            <Button
              variant="ghost"
              size="icon"
              className="absolute left-3 top-1/2 -translate-y-1/2 bg-white/90 backdrop-blur-sm hover:bg-white shadow-lg z-10 rounded-full"
              onClick={prevImage}
            >
              <ChevronLeft className="h-6 w-6" />
            </Button>
            <Button
              variant="ghost"
              size="icon"
              className="absolute right-3 top-1/2 -translate-y-1/2 bg-white/90 backdrop-blur-sm hover:bg-white shadow-lg z-10 rounded-full"
              onClick={nextImage}
            >
              <ChevronRight className="h-6 w-6" />
            </Button>
        ...
        <Button
          variant="ghost"
          size="icon"
          className="absolute right-3 bottom-3 bg-white/90 backdrop-blur-sm hover:bg-white shadow-lg z-10 rounded-full"
          onClick={() => setIsZoomed(true)}
        >
          <Maximize2 className="h-5 w-5" />
        </Button>
        ...
        {/* Image Counter */}
        {validImages.length > 1 && (
          <div className="absolute bottom-3 left-3 rounded-full bg-white/90 backdrop-blur-sm px-3 py-1 text-sm font-medium shadow-lg">
            {selectedImage + 1} / {validImages.length}
          </div>
        )}
      </div>

      {/* Thumbnail Grid - Professional styling */}
      {validImages.length > 1 && (
        <div className="grid grid-cols-5 gap-2">
          {validImages.map((image, index) => (
            <button
              ...
              onClick={() => setSelectedImage(index)}
              className={cn(
                "relative aspect-square overflow-hidden rounded-lg bg-muted transition-all shadow-md hover:shadow-lg",
                selectedImage === index
