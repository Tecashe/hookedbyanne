...
export function CheckoutForm({ userData }: { userData: UserData }) {
  const router = useRouter()
  const { items, getTotal, clearCart } = useCart()
  const [loading, setLoading] = useState(false)
  const [step, setStep] = useState<"shipping" | "payment">("shipping")
  ...
  const handlePointsChange = (checked: boolean) => {
    setUsePoints(checked)
    if (checked && userData) {
      const maxPoints = Math.min(userData.points, Math.floor((subtotal - discount) * POINTS_CONFIG.REDEEM_RATE))
      setPointsToRedeem(maxPoints)
    } else {
      setPointsToRedeem(0)
    }
          ...
          <form onSubmit={handleShippingSubmit} className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {userData && canRedeemPoints(userData.points) && (
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox id="usePoints" checked={usePoints} onCheckedChange={handlePointsChange} />
                      <Label htmlFor="usePoints" className="cursor-pointer">
                        Use {userData.points} reward points (${calculatePointsValue(userData.points).toFixed(2)} off)
                      </Label>
                    </div>
                    {usePoints && (
                      <Alert>
